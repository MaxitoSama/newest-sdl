...
update_status ModuleRender::Update()	
{
	...
	if (App->input->keyboard[SDL_SCANCODE_UP] == KEY_STATE::KEY_REPEAT && App->player->IsEnabled())
		if (App->player->position.y >= (-1864 + SCREEN_HEIGHT) + 121 && App->player->position.y < 121 && camera.y<4824)
			camera.y += speed;

	if (App->input->keyboard[SDL_SCANCODE_DOWN] == KEY_STATE::KEY_REPEAT && App->player->IsEnabled())
